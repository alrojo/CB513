...
def build_model():
    ...
    l_1 = lasagne.layers.DenseLayer(
        l_reshape_a, num_units=N_L1, nonlinearity=None)
    l_1_batchnorm = batchnorm_layer(l_1)
    ...
    l_LSTM_batchnorm = batchnorm_layer(l_reshape_c)
    l_2 = lasagne.layers.DenseLayer(
	lasagne.layers.dropout(l_LSTM_batchnorm, p=0.5), num_units=N_L2, nonlinearity=lasagne.nonlinearities.rectify)
    l_2_batchnorm = batchnorm_layer(l_2)
    # 5. Output Layer
