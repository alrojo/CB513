...
def build_model():
	...
	filter_size=F_CONV_A, stride=1, nonlinearity=lasagne.nonlinearities.very_leaky_rectify)
    l_conv_b = lasagne.layers.Conv1DLayer(
	incoming=l_dim_a, num_filters=N_CONV_B, pad='same',
	filter_size=F_CONV_B, stride=1, nonlinearity=lasagne.nonlinearities.very_leaky_rectify)
    l_conv_c = lasagne.layers.Conv1DLayer(
	incoming=l_dim_a, num_filters=N_CONV_C, pad='same',
	filter_size=F_CONV_C, stride=1, nonlinearity=lasagne.nonlinearities.very_leaky_rectify)
    ...
    l_1 = lasagne.layers.DenseLayer(
        l_reshape_a, num_units=N_L1, nonlinearity=lasagne.nonlinearities.very_leaky_rectify)
...
#    out.eval({sym_x: })
	...
	lasagne.layers.dropout(l_reshape_b, p=0.5), num_units=N_L2, nonlinearity=lasagne.nonlinearities.very_leaky_rectify)
    # 5. Output Layer
