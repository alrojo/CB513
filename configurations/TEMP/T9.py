...
#    batch_size, seq_len, _ = l_in.input_var.shape
    # 3. LSTM Layers
    l_backward = lasagne.layers.LSTMLayer(l_reshape_b, N_LSTM_F, backwards=True)
    l_vertical = lasagne.layers.ConcatLayer([l_reshape_b,l_backward], axis=2)
#    l_sum = lasagne.layers.ConcatLayer([l_in, l_forward],axis=-1)
    l_forward = lasagne.layers.LSTMLayer(l_vertical, N_LSTM_B)
